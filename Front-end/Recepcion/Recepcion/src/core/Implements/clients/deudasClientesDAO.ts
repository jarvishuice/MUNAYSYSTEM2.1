...
export class DeudasClientesDAO implements IDeudasCLientes{
   ...
   async  DeudasClientesBysede(sede: string|null): Promise<DeudaClientesEntity[]> {
        ...
        try{ 
            const response =await fetch(`${this.API}${this.prefijo}/Deudas/${sede}`,{headers:this.headers,});
            if (response.ok) {
    ...
    async DetalleDeudaClientes(sede: string, idCliente: number): Promise<DetalleDeudaCliente[]> {
      try{ 
        const response =await fetch(`${this.API}${this.prefijo}/Deudas/${sede}/${idCliente}`,{headers:this.headers,});
        if (response.ok) {
            const data = await response.json();
            console.log(data)
