...
class WalletEspaciosDAO(ConectionsPsqlInterface,IWallet):
        ...
        try:
            ...
            if conexion['status']== True:
                with self.conn.cursor() as cur :
                    cur.execute(f""" select COALESCE(NULLIF(sum(monto), NULL),0 ) as saldo from wallet_espacios where idcliente={idCliente}""")
                    count=cur. rowcount
                    if count > 0:
                        for i in cur:
                            saldo=i[0]
        ...
        try:
            ...
            if conexion['status']== True:
                with self.conn.cursor() as cur :
                    ...
                    cur.execute(f""" insert into wallet_espacios (idcliente,id,idpago,status,monto) values({wallet.idcliente},{wallet.id},{wallet.idpago},'{wallet.status}',
