...
HAVING SUM(monto) > 0;
    ...
    def generarCierre(self,sede: str):
        try:
            ...
            datos=ReporteCoffeshopEntity(ventasPorProductos=self.__ventasPorProducto__(sede)['response'],deudaCliente=self.__DeudaCleinteBysede__(sede)['response'],detallePedidos=self.__DetallesPedidos__(sede)['response'],walletDisponibles=self.__walletDisponible__(sede)['response'],ventasPorCliente=ventaCLientes,ordenesAbiertasHistoricas=self.__OrdenesHistoricas__(sede)['response'],ordenesAbiertas=self.__OrdenesAbiertas__(sede)['response'],detallePagos=detallePago,puntoCount=self.__countPunto__(sede)['response'])
